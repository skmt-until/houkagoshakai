...
export function RelatedBook({
  ...
  return (
    <div className="bg-white border-4 border-ink rounded-[20px] p-4 flex flex-col gap-3">
      <div className="text-xs font-black tracking-wide text-accent">この記事に関連する本</div>
      <div className="flex gap-3 items-start bg-mint border-[3px] border-ink rounded-2xl p-3">
        <div className="hs-paper-swatch w-[50px] h-[66px] flex-none border-[3px] border-ink rounded-lg flex items-center justify-center text-[22px]">
          {emoji}
        </div>
        <div className="flex flex-col gap-1 flex-1">
          <span className="text-[13.5px] font-black leading-snug text-ink">{title}</span>
          <span className="text-[11px] font-bold text-ink opacity-60">{subtitle}</span>
        </div>
      </div>
      <a
        href={amazonUrl}
        ...
        className="flex items-center justify-center gap-2 bg-accent text-white border-[3px] border-ink rounded-2xl py-2.5 font-black text-[13px] no-underline"
      >
        Amazonで見る →
      </a>
      <div className="text-[10px] font-bold text-ink opacity-50 leading-relaxed">
        ※このリンクはAmazonアソシエイト・プログラムの参加リンクです。当サイトは適格販売により収入を得ることがあります。
      </div>
    </div>
  );
}
